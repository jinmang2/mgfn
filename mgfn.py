...
def MSNSD(features, scores, bs, batch_size, drop_out, ncrops, k):
    ...
    nfea_magnitudes = feat_magnitudes[
        0:batch_size
    ]  # [b/2,32]  # normal feature magnitudes
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(depth):
            if mgfn_type == "fb":
                attention = FOCUS(
                    dim, heads=heads, dim_head=dim_headnumber, local_aggr_kernel=kernel
                )
            elif mgfn_type == "gb":
                attention = GLANCE(
                    dim, heads=heads, dim_head=dim_headnumber, dropout=attention_dropout
                )
            else:
...
class mgfn(nn.Module):
    def __init__(
        ...
        self.to_tokens = nn.Conv1d(
            channels, init_dim, kernel_size=3, stride=1, padding=1
        )
